...
func (d *Driver) CreateVolume(req Request) error {
	...
	if format || volumeNeedsFS(volume){
		if _, err := util.Execute("mkfs", []string{"-t", "ext4", dev}); err != nil {
			return err
		}
		if err := setVolumeHasFs(volume); err != nil {
			return err
		}
	}
...
func (d *Driver) setVolumeHasFS(volume *Volume) error {
	...
	if err := d.ebsService.DeleteTags(volume.EBSID, needFsTag); err != nil {
		log.Errorf("Unable to delete Tag %v(%v) in volume %v.", FILESYSTEM_NEEDED_TAG, volume.Name, volume.EBSID)
		return err
	}
	return nil
}
...
func (d *Driver) volumeNeedsFs(volume *Volume) bool {
	...
	needFs := tags[FILESYSTEM_NEEDED_TAG] != "true" 
	if needFs {
		log.Debugf("Tag '%v' present in %v(%v), %v needs a filesystem.", FILESYSTEM_NEEDED_TAG, 
			volume.Device, volume.Name, volume.EBSID)
	} else {
